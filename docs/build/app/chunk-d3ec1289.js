...
class MDCChip extends MDCComponent {
  ...
  initialize() {
    ...
    const checkmarkEl = this.root_.querySelector(strings$2.CHECKMARK_SELECTOR);
  ...
  getDefaultFoundation() {
    return new MDCChipFoundation(/** @type {!MDCChipAdapter} */ (Object.assign({
      ...
      registerTrailingIconInteractionHandler: (evtType, handler) => {
        const trailingIconEl = this.root_.querySelector(strings$2.TRAILING_ICON_SELECTOR);
        if (trailingIconEl) {
      ...
      notifyInteraction: () => this.emit(strings$2.INTERACTION_EVENT, {chip: this}, true /* shouldBubble */),
      notifyTrailingIconInteraction: () => this.emit(
        strings$2.TRAILING_ICON_INTERACTION_EVENT, {chip: this}, true /* shouldBubble */),
      notifyRemoval: () => this.emit(strings$2.REMOVAL_EVENT, {chip: this, root: this.root_}, true /* shouldBubble */),
...
class MDCIconButtonToggleFoundation extends MDCFoundation {
  ...
  refreshToggleData() {
    this.toggleOnData_ = {
      label: this.adapter_.getAttr(strings$9.DATA_TOGGLE_ON_LABEL),
      content: this.adapter_.getAttr(strings$9.DATA_TOGGLE_ON_CONTENT),
      cssClass: this.adapter_.getAttr(strings$9.DATA_TOGGLE_ON_CLASS),
    };
    this.toggleOffData_ = {
      label: this.adapter_.getAttr(strings$9.DATA_TOGGLE_OFF_LABEL),
      content: this.adapter_.getAttr(strings$9.DATA_TOGGLE_OFF_CONTENT),
      cssClass: this.adapter_.getAttr(strings$9.DATA_TOGGLE_OFF_CLASS),
    };
  }
...
class MDCMenuFoundation extends MDCFoundation {
  ...
  close(evt = null) {
    const targetIsDisabled = evt ?
      this.adapter_.getAttributeForEventTarget(evt.target, strings$c.ARIA_DISABLED_ATTR) === 'true' :
      false;
  ...
  getDefaultFoundation() {
    return new MDCNotchedOutlineFoundation({
      ...
      setOutlinePathAttr: (value) => {
        const path = this.root_.querySelector(strings$d.PATH_SELECTOR);
        path.setAttribute('d', value);
      },
      getIdleOutlineStyleValue: (propertyName) => {
        const idleOutlineElement = this.root_.parentNode.querySelector(strings$d.IDLE_OUTLINE_SELECTOR);
...
class MDCSelect extends MDCComponent {
  ...
  initialize(
    ...
    this.nativeControl_ = this.root_.querySelector(strings$f.NATIVE_CONTROL_SELECTOR);
    const labelElement = this.root_.querySelector(strings$f.LABEL_SELECTOR);
    if (labelElement) {
      this.label_ = labelFactory(labelElement);
    }
    const lineRippleElement = this.root_.querySelector(strings$f.LINE_RIPPLE_SELECTOR);
    if (lineRippleElement) {
      this.lineRipple_ = lineRippleFactory(lineRippleElement);
    }

    if (this.root_.classList.contains(cssClasses$g.BOX)) {
...
class MDCSlider extends MDCComponent {
  ...
  getDefaultFoundation() {
    return new MDCSliderFoundation(
      /** @type {!MDCSliderAdapter} */ ({
        ...
        notifyInput: () => {
          this.emit(strings$g.INPUT_EVENT, this);
        },
        notifyChange: () => {
          this.emit(strings$g.CHANGE_EVENT, this);
        },
        ...
        setLastTrackMarkersStyleProperty: (propertyName, value) => {
          ...
          const lastTrackMarker = this.root_.querySelector(strings$g.LAST_TRACK_MARKER_SELECTOR);
  ...
  initialSyncWithDOM() {
    const origValueNow = parseFloat(this.root_.getAttribute(strings$g.ARIA_VALUENOW));
    ...
    this.disabled = (
      this.root_.hasAttribute(strings$g.ARIA_DISABLED) &&
      this.root_.getAttribute(strings$g.ARIA_DISABLED) !== 'false'
    );
...
class MDCSnackbarFoundation extends MDCFoundation {
  ...
  show(data) {
    ...
    const {ACTIVE, MULTILINE, ACTION_ON_BOTTOM} = cssClasses$i;
...
class MDCTabFoundation extends MDCFoundation {
  ...
  setActive(isActive) {
    ...
    if (this.isActive_) {
      this.adapter_.addClass(cssClasses$j.ACTIVE);
    } else {
      this.adapter_.removeClass(cssClasses$j.ACTIVE);
    }
  }
...
class MDCTab extends MDCComponent {
  ...
  initialSyncWithDOM() {
    this.isActive = this.root_.classList.contains(cssClasses$j.ACTIVE);
  }
...
class MDCTabBarFoundation extends MDCFoundation {
  ...
  destroy() {
    this.adapter_.removeClass(cssClasses$k.UPGRADED);
...
class MDCTextFieldHelperTextFoundation extends MDCFoundation {
  ...
  showToScreenReader() {
    this.adapter_.removeAttr(strings$m.ARIA_HIDDEN);
  }
  ...
  setValidity(inputIsValid) {
    const helperTextIsPersistent = this.adapter_.hasClass(cssClasses$n.HELPER_TEXT_PERSISTENT);
    const helperTextIsValidationMsg = this.adapter_.hasClass(cssClasses$n.HELPER_TEXT_VALIDATION_MSG);
    const validationMsgNeedsDisplay = helperTextIsValidationMsg && !inputIsValid;

    if (validationMsgNeedsDisplay) {
      this.adapter_.setAttr(strings$m.ROLE, 'alert');
    } else {
      this.adapter_.removeAttr(strings$m.ROLE);
    }
   ...
   */
  hide_() {
    this.adapter_.setAttr(strings$m.ARIA_HIDDEN, 'true');
  }
}
...
class MDCTextFieldIconFoundation extends MDCFoundation {
  ...
  setDisabled(disabled) {
    ...
    if (disabled) {
      ...
      this.adapter_.setAttr('role', strings$n.ICON_ROLE);
    }
  }
...
class MDCTextFieldFoundation extends MDCFoundation {
  ...
  notchOutline(openNotch) {
    ...
    if (openNotch) {
      const isDense = this.adapter_.hasClass(cssClasses$m.DENSE);
...
class MDCTextField extends MDCComponent {
  ...
  initialize(
    ...
    this.input_ = this.root_.querySelector(strings$l.INPUT_SELECTOR);
    const labelElement = this.root_.querySelector(strings$l.LABEL_SELECTOR);
    if (labelElement) {
      this.label_ = labelFactory(labelElement);
    }
    const lineRippleElement = this.root_.querySelector(strings$l.LINE_RIPPLE_SELECTOR);
    if (lineRippleElement) {
      this.lineRipple_ = lineRippleFactory(lineRippleElement);
    }
    const outlineElement = this.root_.querySelector(strings$l.OUTLINE_SELECTOR);
    if (outlineElement) {
      this.outline_ = outlineFactory(outlineElement);
    }
    if (this.input_.hasAttribute(strings$l.ARIA_CONTROLS)) {
      const helperTextElement = document.getElementById(this.input_.getAttribute(strings$l.ARIA_CONTROLS));
      if (helperTextElement) {
        this.helperText_ = helperTextFactory(helperTextElement);
      }
    }
    const iconElement = this.root_.querySelector(strings$l.ICON_SELECTOR);
    if (iconElement) {
      this.icon_ = iconFactory(iconElement);
    }

    this.ripple = null;
    if (this.root_.classList.contains(cssClasses$m.BOX)) {
      const MATCHES = getMatchesProperty(HTMLElement.prototype);
      const adapter =
    ...
    return new MDCTextFieldFoundation(
      /** @type {!MDCTextFieldAdapter} */ (Object.assign({
        ...
        registerValidationAttributeChangeHandler: (handler) => {
          ...
          const targetNode = this.root_.querySelector(strings$l.INPUT_SELECTOR);
        ...
        isFocused: () => {
          return document.activeElement === this.root_.querySelector(strings$l.INPUT_SELECTOR);
        },
  ...
  fixedScrollHandler_() {
    ...
    if (currentScroll <= 0) {
      if (this.wasScrolled_) {
        this.adapter_.removeClass(cssClasses$p.FIXED_SCROLLED_CLASS);
        this.wasScrolled_ = false;
      }
    } else {
      if (!this.wasScrolled_) {
        this.adapter_.addClass(cssClasses$p.FIXED_SCROLLED_CLASS);
...
class MDCShortTopAppBarFoundation extends MDCTopAppBarBaseFoundation {
  ...
  init() {
    super.init();
    const isAlwaysCollapsed = this.adapter_.hasClass(cssClasses$p.SHORT_COLLAPSED_CLASS);
...
class MDCTopAppBar extends MDCComponent {
  ...
  initialize(
    ...
    this.navIcon_ = this.root_.querySelector(strings$p.NAVIGATION_ICON_SELECTOR);

    // Get all icons in the toolbar and instantiate the ripples
    const icons = [].slice.call(this.root_.querySelectorAll(strings$p.ACTION_ITEM_SELECTOR));
    if (this.navIcon_) {
  ...
  getDefaultFoundation() {
    ...
    const adapter = /** @type {!MDCTopAppBarAdapter} */ (Object.assign({
      ...
      notifyNavigationIconClicked: () => {
        this.emit(strings$p.NAVIGATION_EVENT, {});
      },
      registerScrollHandler: (handler) => window.addEventListener('scroll', handler),
      deregisterScrollHandler: (handler) => window.removeEventListener('scroll', handler),
      registerResizeHandler: (handler) => window.addEventListener('resize', handler),
      ...
      getViewportScrollY: () => window.pageYOffset,
      getTotalActionItems: () =>
        this.root_.querySelectorAll(strings$p.ACTION_ITEM_SELECTOR).length,
    })
    );

    /** @type {!MDCTopAppBarBaseFoundation} */
    let foundation;
    if (this.root_.classList.contains(cssClasses$p.SHORT_CLASS)) {
      foundation = new MDCShortTopAppBarFoundation(adapter);
    } else if (this.root_.classList.contains(cssClasses$p.FIXED_CLASS)) {
...
mdcAutoInit.register('MDCPersistentDrawer', MDCPersistentDrawer);
...
mdcAutoInit.register('MDCGridList', MDCGridList);
...
export { MDCDialogFoundation as a, MDCDialog as b, MDCTopAppBar as c, MDCTopAppBarBaseFoundation as d, MDCTopAppBarFoundation as e, MDCFixedTopAppBarFoundation as f, MDCShortTopAppBarFoundation as g };
